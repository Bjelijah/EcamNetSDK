...
import com.howell.baidumap.GPSMapActivity;
import com.howell.baidumap.RMCMapActivity;
    ...
    @OnClick(R.id.btn_record) void clickRecord(){
        new Thread(){
            ...
            public void run() {
                ...
                Date now = new Date();
                String end = MyUtil.Date2ISODate(now);
                Calendar c = Calendar.getInstance();
                c.setTime(now);
                c.add(Calendar.DAY_OF_MONTH,-1);
                Date yesterday =  c.getTime();
                String beg = MyUtil.Date2ISODate(yesterday);
                try {
